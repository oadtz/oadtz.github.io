...
function App() {
  ...
  return (
    <div className={`relative w-screen h-[100dvh] overflow-hidden transition-colors duration-500 ${isNightMode ? 'bg-[#0a0a2a]' : 'bg-sky-300'}`}>
      ...
      <a
        href="/"
        aria-label="Return to main Thanapat Pirmphol site"
        className={`fixed top-3 right-3 md:top-5 md:right-5 z-30 font-pixel text-[10px] md:text-xs px-3 py-2 rounded border-2 border-b-4 active:border-b-2 active:translate-y-0.5 transition-all ${isNightMode
          ? 'bg-slate-900/85 hover:bg-slate-800 text-white border-white/60 shadow-[0_0_12px_rgba(255,255,255,0.18)]'
          : 'bg-white/85 hover:bg-white text-black border-black shadow-[3px_3px_0_rgba(0,0,0,0.22)]'
          }`}
      >
        MAIN SITE
      </a>
